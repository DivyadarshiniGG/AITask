...
GitHub isn't meant for large data files. Download it from the original source link provided in the
assignment and place it in `csv_files/` before running the notebook:

- Historical Data: [Google Drive link from assignment]
- Fear & Greed Index: [Google Drive link from assignment] (small file, included here for convenience)
...
```
Run all cells top to bottom — the notebook expects `historical_data.csv` and `csv_files/fear_greed_index.csv`
to sit alongside it (adjust paths if you keep them in `csv_files/`).
